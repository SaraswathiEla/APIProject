...
public class postGetUpdaeAndDelete {
	@Test
	public void getandDeleteProject()
	{
		...
		ProjectLibrary proj=new ProjectLibrary("sar","absv","Created",25);

		
		
		given().body(proj).pathParam("projectId", acId)
		.when().put("projects/{projectId}");
		
		
		
		
		resp.jsonPath().get("projectId");
 
		
		given().pathParam("projectId", acId)
		.when().delete("/projects/{projectId}").then().statusCode(204).log().all();
